...
OUTPUT_CSV = os.path.join(SOURCE_DIR, "experiment_results_bf.csv")

# Compile commands
COMPILE_CMDS = [
    (f"g++ -O3 -std=c++17 {os.path.join(SOURCE_DIR, 'bf_seq.cpp')} -o {os.path.join(SOURCE_DIR, 'bf_seq')}", "bf_seq"),
    (f"mpicxx -O3 -std=c++17 {os.path.join(SOURCE_DIR, 'bf_mpi.cpp')} -o {os.path.join(SOURCE_DIR, 'bf_mpi')}", "bf_mpi"),
    (f"nvcc -O3 -std=c++17 {os.path.join(SOURCE_DIR, 'bf_cuda.cu')} -o {os.path.join(SOURCE_DIR, 'bf_cuda')}", "bf_cuda")
]

def compile_code():
    print("Compiling brute force implementations...")
...
def run_experiment():
    ...
    for pat_file, txt_file in datasets:
        ...
        # 1. Sequential
        cmd = [os.path.join(SOURCE_DIR, 'bf_seq'), pat_file, txt_file]
        try:
        ...
        # 2. MPI (4 ranks)
        cmd = ["mpirun", "-np", "4", "--mca", "mpi_cuda_support", "0", os.path.join(SOURCE_DIR, 'bf_mpi'), pat_file, txt_file]
        try:
            res = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
            if res.returncode == 0:
                m, t = parse_output(res.stdout)
        ...
        # 3. CUDA
        cmd = [os.path.join(SOURCE_DIR, 'bf_cuda'), pat_file, txt_file]
        try:
            res = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
            if res.returncode == 0:
                m, t = parse_output(res.stdout)
